...
def convertChiffreRomain(i):
    ...
    return result # retourne le chiffre Romain
...
def generateInseeDir(path):
    ...
    # Tester l'existence du dossier passé en paramètre
    if (os.path.exists(path) == False):
        os.mkdir(path) # Création du dossier si il n'existe pas
...
def readDir(path, outputFile):
    ...
    fw = open(outputFile, 'w') # Ouverture du fichier texte en ecriture
